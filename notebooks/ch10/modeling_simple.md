...
Normally, we would denote our model as $f_{\hat{\theta}}(x) = \hat{\theta_0}$ where $\hat{\theta_0} = 15$, but since our model is not a function of any independent variables, we represent our estimate as $\hat{\theta_0}$. Later, we will use the $f_{\hat{\theta}}(x)$ notation for more complicated models.
