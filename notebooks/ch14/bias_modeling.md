...
We denote the process that generates the true population data as $ f_\theta(x) $. Our observed outcomes $ \gamma $ were generated by our population process plus some random noise in data collection: $ \gamma_i = f_\theta(z_i) + \epsilon $.  The random noise $ \epsilon $ is a random variable with a mean of zero: $ \mathbb{E}[\epsilon] = 0 $.
