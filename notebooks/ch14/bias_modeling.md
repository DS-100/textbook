...
We denote the process that generates the true population data as $ f_\theta $. Our observed outcomes $ y $ were generated by our population process plus some random noise in data collection: $ y_i = h(X_i) + \epsilon $. $ h(x) $ is the true function generating the data and the random noise $ \epsilon $ is a random variable with a mean of zero: $ \mathbb{E}[\epsilon] = 0 $.
...
We start with the mean squared error
...
We then substitute $h(z) + \epsilon$ for $\gamma$:
